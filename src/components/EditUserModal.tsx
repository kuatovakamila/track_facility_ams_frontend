...
const EditUserModal = ({ isOpen, onClose, onSave, user }: EditUserModalProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Добавляем пароль только если пользователь хочет его изменить
  ...
  const handleDeactivateToggle = () => {
    ...
    handleInputChange('is_active', newActiveState);
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          {/* Статус активности - выделенный блок */}
          <div className="bg-gray-50 rounded-lg p-4 border">
            <div className="flex items-center justify-between">
              <div>
                <label className="block text-sm font-medium text-gray-700">
                  {formData.is_active ? 'Активный пользователь' : 'Деактивированный пользователь'}
                </label>
                <p className="text-xs text-gray-500">
                  {formData.is_active 
                    ? 'Пользователь может входить в систему' 
                    : 'Пользователь не может входить в систему'
                  }
                </p>
              </div>
              <button
          ...
          <div className="border-t pt-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <button
                ...
                className={`relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 ${
                  changePassword ? 'bg-blue-600' : 'bg-gray-200'
                }`}
                disabled={loading}
              >
                <span
